...
def github_webhook():
    ...
    if event == "pull_request":
        ...
        if response.status_code == 201:
            print("✅ PR review success!")
        else:
            print(f"❌ PR review failed: {response.status_code}, {response.text}")
        ...
        if response.status_code == 201:
            print("✅ Issue review success！")
        else:
            print(f"❌ Issue review failed: {response.status_code}, {response.text}")
